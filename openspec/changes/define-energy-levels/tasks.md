...
- [ ] 2.4 Add a fixture vector that discriminates the two surplus compositions. D27 amended D10 to `max(0, grid + reclaimable)`, and **no vector in `fixtures/` tells the two apart** — the corpus's battery scenario has grid at exactly 0, where the literal sum and the netted figure agree. The new vector needs the site importing while the battery charges (design.md §8, `define-engine-contract` design.md §11)
